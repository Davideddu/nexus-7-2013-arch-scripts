...
ROM_NAME="ArchLinux"
CHROOT="/data/media/0/multirom/roms/$ROM_NAME/root"
...
echo "Download errors can usually be ignored if it has downloaded successfully once."
...
/data/local/tmp/arch/busybox wget -c "http://nl.mirror.archlinuxarm.org/os/ArchLinuxARM-trimslice-latest.tar.gz" -O "/sdcard/download/alarm-flo.tar.gz"
...
/data/local/tmp/arch/busybox mkdir -p ${CHROOT}
/data/local/tmp/arch/busybox gunzip "/sdcard/download/alarm-flo.tar.gz" -c | /data/local/tmp/arch/busybox tar x -f - -C ${CHROOT} || exit 1
...
echo "Setting SELinux policy back to \"Enforcing\"..."
